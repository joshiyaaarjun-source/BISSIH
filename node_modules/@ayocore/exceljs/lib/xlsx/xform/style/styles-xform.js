...
class StylesXform extends BaseXform {
  constructor(initialise, cacheMode) {
    super();
    this.cacheMode = cacheMode || CACHE_MODES.WEAK_MAP;
  ...
  init() {
    ...
    this.styleMap = this.cacheMode === CACHE_MODES.WEAK_MAP ? new WeakMap() : new Map();
  }
  ...
  addStyleModel(model, cellType) {
    ...
    if (this.cacheMode !== CACHE_MODES.NO_CACHE && this.styleMap) {
      let cacheKey;
      switch (this.cacheMode) {
        case CACHE_MODES.JSON_MAP:
          cacheKey = JSON.stringify(model);
          break;
        case CACHE_MODES.FAST_MAP:
      ...
      if (this.styleMap.has(cacheKey)) {
        return this.styleMap.get(cacheKey);
      }
      // store cacheKey for later use when setting
    ...
    if (this.cacheMode !== CACHE_MODES.NO_CACHE && this.styleMap) {
      const cacheKey = this._lastCacheKey !== undefined ? this._lastCacheKey : model;
