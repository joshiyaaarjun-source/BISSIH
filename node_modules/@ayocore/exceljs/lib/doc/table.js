...
class Table {
  ...
  validate() {
    ...
    // autoFilterRef is a range that includes the header row only
    if (table.headerRow) {
  ...
  _updateTableRef() {
    ...
    const height = (table.headerRow ? 1 : 0) + table.rows.length + (table.totalsRow ? 1 : 0);

    table.tableRef = colCache.encode(row, col, row + height - 1, col + width - 1);
    if (table.headerRow) {
      table.autoFilterRef = colCache.encode(row, col, row, col + width - 1);
    }
  }
  ...
  set autoFilterRef(value) {
    this.table.autoFilterRef = value;
  }
  /* eslint-enable lines-between-class-members */
}
