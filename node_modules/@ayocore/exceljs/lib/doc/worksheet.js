...
Please leave feedback at https://github.com/exceljs/exceljs/discussions/2575`
    ...
    // Assign global pivot table number for unique file naming
  ...
  set model(value) {
    ...
    this.tables = value.tables.reduce((tables, table) => {
      // fix up table data from loaded model
      if (!table.ref && table.tableRef) {
        table.ref = table.tableRef.split(':')[0];
      }
      if (!table.rows) {
        table.rows = [];
      }
      if (table.columns && table.columns.length && !table.headerRow) {
        // detect headerRow from column names presence
        if (table.columns.some(c => c.name)) {
          table.headerRow = true;
        }
      }
      if (table.autoFilterRef && table.columns) {
        table.columns.forEach(c => {
          if (c.filterButton === undefined) {
            c.filterButton = true;
          }
        });
      }
